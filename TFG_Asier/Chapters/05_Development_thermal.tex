...
The external configuration is presented in
...
identifies the equipment groups to which an internal heat load is applied.
\autoref{tab:subsystem-classification}.

\input{./Tables/tabla_subsistemas.tex}
...
interaction with solar and infrared radiation. These definitions are common to all analysis cases. Consequently, changes in the calculated results are 
...
\acrshort{mli} foil has an independent bulk definition whose conductivity is 
set to zero. Through-thickness heat transfer is represented separately by 
temperature-dependent effective conductance properties assigned to the 
\acrshort{mli} surfaces.
...
\input{./Tables/tabla_materiales.tex}
...
radiator surface, and the solar cells use a separate coating definition. 
The external and internal \acrshort{mli} surfaces use the 
\texttt{MLI\_ext} and \texttt{MLI\_int} definitions, respectively, while 
Kapton coated with \acrfull{ito} is assigned to the rear surfaces of the 
solar panels. An independent property is assigned to the infrared lens.
...
\input{./Tables/tabla_propiedades_termo_opticas.tex}
...
\input{./Tables/tabla_cargas_internas_5_4.tex}

The node definitions and thermal couplings remain unchanged in all transient 
cases. Only the environmental conditions and internal heat loads are modified. Consequently, each node and thermal coupling 
...
\input{./Tables/tabla_casos_transitorios.tex}

All three simulations use the common model configuration defined above. Only
the radiative environment and internal heat loads change between cases.
Equivalent nodes and exported result fields therefore retain the same physical
meaning, allowing direct comparison with the nominal case.
...
After each simulation, the corresponding result set is exported from
\acrshort{esatan-tms} as a separate \texttt{.TMD} file. Each file stores the
time vector and the nodal results selected for post-processing, including
...
\input{./Tables/tabla_resumen_tmd.tex}
...
\(284 \times 101 = 28\,684\) values before the conductor results and the remaining
nodal result fields are considered.
